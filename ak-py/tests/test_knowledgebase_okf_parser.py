...
class TestParseConceptSkips:
    ...
    def test_a_comma_in_the_path_is_refused_because_fetch_ids_split_on_it(self):
        concept, diagnostics = OKFParserUtil.parse_concept("tables/a,b.md", document("type: Table"), body_complete=True)
    ...
    def test_a_document_with_no_frontmatter_block_is_skipped(self):
        concept, diagnostics = OKFParserUtil.parse_concept("a.md", "# Just a heading\n", body_complete=True)
        assert concept is None
    ...
    def test_frontmatter_that_is_not_valid_yaml_is_skipped(self):
        concept, diagnostics = OKFParserUtil.parse_concept("a.md", document("type: [unclosed"), body_complete=True)
        assert concept is None
        assert codes(diagnostics) == [DiagnosticCode.UNPARSEABLE_FRONTMATTER.value]

    @pytest.mark.parametrize("frontmatter", ["- a\n- b", "just a string", ""])
    def test_frontmatter_that_is_not_a_mapping_is_skipped(self, frontmatter):
        concept, diagnostics = OKFParserUtil.parse_concept("a.md", document(frontmatter), body_complete=True)
        assert concept is None
        assert codes(diagnostics) == [DiagnosticCode.UNPARSEABLE_FRONTMATTER.value]
...
class TestParseConceptTolerance:
    def test_an_unknown_type_value_is_kept_verbatim(self):
        concept, diagnostics = OKFParserUtil.parse_concept("a.md", document("type: Some Future Kind"), body_complete=True)
        assert concept.type == "Some Future Kind"
        assert diagnostics == []

    def test_unknown_keys_land_in_extra_untouched(self):
        concept, diagnostics = OKFParserUtil.parse_concept(
            "a.md", document("type: Table\nowner: analytics\nreview: {every: 30d}"), body_complete=True
        )
    ...
    def test_the_computation_family_collapses_into_computation_and_not_into_extra(self):
        ...
        concept, _ = OKFParserUtil.parse_concept("a.md", document(frontmatter), body_complete=True)
    ...
    def test_missing_optional_fields_take_defaults_with_no_diagnostic(self):
        concept, diagnostics = OKFParserUtil.parse_concept("a.md", document("type: Table"), body_complete=True)
    ...
    def test_a_scalar_tags_value_becomes_a_one_element_list(self):
        concept, diagnostics = OKFParserUtil.parse_concept("a.md", document("type: Table\ntags: sales"), body_complete=True)
...
class TestVersionZeroTwoOnly:
    ...
    def test_a_body_citations_list_stays_ordinary_body_text(self):
        ...
        concept, _ = OKFParserUtil.parse_concept("a.md", document("type: Table", body), body_complete=True)
...
class TestBodyHandling:
    def test_a_complete_body_is_retained_with_its_links(self):
        concept, _ = OKFParserUtil.parse_concept("tables/orders.md", document("type: Table", "see [c](./customers.md)"), body_complete=True)
...
class TestTokenise:
    def test_tokens_are_lowercased_split_on_non_alphanumerics_and_at_least_two_characters(self):
        assert OKFParserUtil.tokenise("Orders_2026: a per-customer TABLE!") == {"orders", "2026", "per", "customer", "table"}
...
class TestTrust:
    def test_no_verified_entries_is_unverified(self):
        assert OKFParserUtil.derive_trust([]) is TrustTier.UNVERIFIED

    def test_an_agent_actor_is_machine_confirmed(self):
        assert OKFParserUtil.derive_trust([{"by": "reference_agent/gemini-2.5-pro"}]) is TrustTier.MACHINE_CONFIRMED

    def test_one_human_actor_among_agents_is_human_reviewed(self):
        assert OKFParserUtil.derive_trust([{"by": "process:nightly"}, {"by": "human:jsmith"}]) is TrustTier.HUMAN_REVIEWED

    def test_a_malformed_entry_does_not_upgrade_the_tier(self):
        assert OKFParserUtil.derive_trust([{"at": "2026-01-01"}]) is TrustTier.MACHINE_CONFIRMED
...
class TestStaleness:
    def test_no_deadline_is_never_stale(self):
        assert OKFParserUtil.is_stale(None, NOW) == (False, [])

    def test_a_deadline_in_the_future_is_not_stale(self):
        assert OKFParserUtil.is_stale("2026-12-01T00:00:00Z", NOW) == (False, [])

    def test_a_deadline_in_the_past_is_stale(self):
        stale, diagnostics = OKFParserUtil.is_stale("2026-01-01T00:00:00Z", NOW)
        assert (stale, diagnostics) == (True, [])

    def test_a_naive_deadline_is_read_as_utc(self):
        assert OKFParserUtil.is_stale("2026-01-01T00:00:00", NOW)[0] is True

    def test_an_unparseable_deadline_is_reported_and_never_assumed_stale(self):
        stale, diagnostics = OKFParserUtil.is_stale("next tuesday", NOW)
    ...
    def test_the_concept_path_is_stamped_onto_a_staleness_diagnostic(self):
        ...
        _, diagnostics = OKFParserUtil.parse_concept("tables/orders.md", document("type: Table\nstale_after: soon"), body_complete=True)
...
class TestExtractLinks:
    ...
    def test_a_link_escaping_the_bundle_is_dropped_with_a_diagnostic(self):
        links, diagnostics = OKFParserUtil.extract_links("tables/orders.md", "see [x](../../secrets.md)")
        assert links == []
    ...
    def test_repeated_links_yield_one_entry_in_first_occurrence_order(self):
        ...
        assert OKFParserUtil.extract_links("tables/orders.md", body)[0] == ["tables/b.md", "tables/a.md"]

    def test_a_root_level_concept_resolves_relative_links_against_the_bundle_root(self):
        assert OKFParserUtil.extract_links("orders.md", "[x](./customers.md)")[0] == ["customers.md"]
...
class TestReservedFiles:
    ...
    def test_both_names_are_reserved_at_every_level(self, path):
        assert OKFParserUtil.is_reserved(path) is True
    ...
    def test_an_ordinary_concept_is_not_reserved(self, path):
        assert OKFParserUtil.is_reserved(path) is False

    def test_an_index_with_no_frontmatter_is_all_body(self):
        body, version, diagnostics = OKFParserUtil.parse_index("index.md", "# Sales\n- [orders](/tables/orders.md)\n", is_root=True)
        assert body == "# Sales\n- [orders](/tables/orders.md)\n"
        assert (version, diagnostics) == (None, [])

    def test_the_root_index_may_declare_the_version(self):
        body, version, diagnostics = OKFParserUtil.parse_index("index.md", document(f'okf_version: "{OKF_VERSION}"', "# Sales\n"), is_root=True)
        assert (body, version, diagnostics) == ("# Sales\n", OKF_VERSION, [])

    def test_other_keys_in_the_root_index_are_unrecognised_content_not_an_error(self):
        _, version, diagnostics = OKFParserUtil.parse_index("index.md", document("curator: analytics"), is_root=True)
        assert (version, diagnostics) == (None, [])

    def test_a_version_this_reader_does_not_target_is_reported_and_the_bundle_still_loads(self):
        body, version, diagnostics = OKFParserUtil.parse_index("index.md", document('okf_version: "0.1"', "# Sales\n"), is_root=True)
    ...
    def test_a_non_root_index_carrying_frontmatter_is_reported_and_its_body_still_used(self):
        body, version, diagnostics = OKFParserUtil.parse_index("tables/index.md", document("okf_version: '0.2'", "# Tables\n"), is_root=False)
    ...
    def test_unparseable_index_frontmatter_still_serves_the_listing(self):
        body, version, diagnostics = OKFParserUtil.parse_index("index.md", document("okf_version: [unclosed", "# Sales\n"), is_root=True)
        assert (body, version) == ("# Sales\n", None)
        assert codes(diagnostics) == [DiagnosticCode.UNPARSEABLE_FRONTMATTER.value]
...
class TestDiagnosticCoverage:
    ...
    def test_every_parser_reachable_code_is_actually_emitted(self):
        ...
        for path, data in [("a,b.md", document("type: Table")), ("a.md", "no block"), ("a.md", document("title: x"))]:
            emitted.update(codes(OKFParserUtil.parse_concept(path, data, body_complete=True)[1]))
        emitted.update(
            codes(OKFParserUtil.parse_concept("a.md", document("type: T\ntags: x\nstale_after: soon", "[e](../../x.md)"), body_complete=True)[1])
        )
        emitted.update(codes(OKFParserUtil.parse_index("index.md", document("okf_version: '0.1'"), is_root=True)[2]))
        emitted.update(codes(OKFParserUtil.parse_index("t/index.md", document("a: b"), is_root=False)[2]))
