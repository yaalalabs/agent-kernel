...
The handler polls full records (``get_record``) rather than bodies, so a stored status_code
reaches ``_build_sync_response`` on every queue-backed surface.
"""
...
def client_and_store():
    ...
    config.execution.response_store.retry_count = 1
