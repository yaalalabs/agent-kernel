"""Messenger adapter: webhook delivery -> InboundRequest, and agent reply -> Send API calls."""
...
from agentkernel.integration.messenger.adapter import MessengerInboundAdapter, MessengerOutboundAdapter
...
def _config(monkeypatch):
    ...
    monkeypatch.setenv("AK_MESSENGER__ACCESS_TOKEN", "token")
    monkeypatch.setenv("AK_MESSENGER__VERIFY_TOKEN", "verify-me")
    monkeypatch.setenv("AK_MESSENGER__APP_SECRET", APP_SECRET)
    monkeypatch.setenv("AK_MESSENGER__AGENT", "helper")
    AKConfig._reset()
...
def _inbound():
    adapter = MessengerInboundAdapter()
    adapter._api = _api_stub()
    return adapter


def _outbound():
    adapter = MessengerOutboundAdapter()
    adapter._api = _api_stub()
    return adapter
...
class TestParse:
    ...
    async def test_a_text_message_becomes_a_normalized_request(self):
        ...
        # Messenger has no thread: the sender is the conversation.
    ...
    async def test_a_postback_is_read_as_its_title(self):
        ...
        assert request.request_id == f"messenger:{SENDER}:START"

    @pytest.mark.asyncio
    async def test_delivery_and_read_receipts_are_not_messages(self):
        events = [{"sender": {"id": SENDER}, "delivery": {"watermark": 1}}, {"sender": {"id": SENDER}, "read": {"watermark": 1}}]
    ...
    async def test_a_foreign_object_type_is_ignored(self):
        assert (await _inbound().parse(_fake_request({"object": "instagram", "entry": []}))).requests == []
    ...
    async def test_an_image_attachment_travels_as_a_stored_reference(self, monkeypatch):
        ...
        AKConfig._reset()
        event = _message_event(attachments=[{"type": "image", "payload": {"url": "https://cdn.fb/photo.jpg"}}])
    ...
    @pytest.mark.asyncio
    async def test_an_attachment_failure_is_explained_to_the_agent(self):
        adapter = _inbound()
        adapter._api.download_attachment = AsyncMock(side_effect=RuntimeError("cdn down"))
        event = _message_event(text="", attachments=[{"type": "image", "payload": {"url": "https://cdn.fb/photo.jpg"}}])

        [request] = (await adapter.parse(_fake_request(_delivery(event)))).requests

        assert "couldn't process one of your attachments" in request.requests[0].prompt
...
class TestDeliver:
    ...
    @pytest.mark.asyncio
    async def test_a_long_reply_is_split_at_the_messenger_limit(self):
        adapter = _outbound()

        await adapter.deliver(AgentReplyText(response="x" * 2500), {"recipient_id": SENDER})

        assert [len(c) for c in adapter._api.send_message.await_args.args[1]] == [2000, 500]
...
class TestRoute:
    def test_a_signed_delivery_is_enqueued(self):
        ...
        response = TestClient(app).post("/messenger/webhook", content=body, headers={"x-hub-signature-256": signature})

        assert response.status_code == 200
        [message] = transport.create_consumer(QueueName.INPUT).fetch(1, 1.0)
        assert message.attributes["integration"] == "messenger"
