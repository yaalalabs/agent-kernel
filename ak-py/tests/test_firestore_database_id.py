...
class TestFirestoreDatabaseIdBugCondition(unittest.TestCase):
    ...
    def test_named_database_connection_with_database_id(self, mock_client_class):
        ...
        self.assertEqual(call_kwargs["database"], "test-database-name",
                        "database parameter should match the configured database_id")
    ...
    def test_config_has_database_id_field(self, mock_client_class):
        ...
        self.assertTrue(hasattr(cfg, "database_id"),
                       "EXPECTED FAILURE: _FirestoreConfig missing database_id field. "
                       "This confirms the bug - no way to configure database name.")
        
        # Verify the value is correctly read from environment
        self.assertEqual(cfg.database_id, "test-database-name",
                        "database_id should be read from AK_SESSION__FIRESTORE__DATABASE_ID")
    ...
    def test_driver_stores_database_id(self, mock_client_class):
        ...
        self.assertTrue(hasattr(driver, "_database_id"),
        ...
        self.assertEqual(driver._database_id, "test-database-name",
                        "Driver should store database_id from config")
...
class TestFirestorePreservation(unittest.TestCase):
    ...
    **Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5, 3.6**
    ...
    def test_project_id_passed_to_client(self, mock_client_class):
        ...
        self.assertIn("project", call_kwargs,
                     "Baseline behavior: project parameter must be passed when project_id is configured")
    ...
    def test_ttl_configuration_sets_expiry_time(self, mock_client_class):
        ...
        self.assertIn("expiry_time", data,
                     "Baseline behavior: expiry_time must be set when TTL is configured")
        self.assertIsInstance(data["expiry_time"], datetime.datetime,
                            "expiry_time should be a datetime object")
        self.assertIn("key1", data,
                     "Session key should be in the data")
    ...
    def test_no_database_id_in_unfixed_config(self, mock_client_class):
        ...
        self.assertFalse(hasattr(cfg, "database_id"),
                        "Current state: _FirestoreConfig does not have database_id field. "
    ...
    def test_reserved_fields_excluded_from_keys(self, mock_client_class):
        ...
        self.assertIn("key1", keys)
        self.assertIn("key2", keys)
        self.assertNotIn("expiry_time", keys,
                        "Baseline behavior: expiry_time should be excluded from session keys")
...
class TestFirestorePreservationPropertyBased(unittest.TestCase):
    ...
    def test_various_ttl_values(self, mock_client_class):
        ...
        for ttl in ttl_values:
            with self.subTest(ttl=ttl):
                ...
                if ttl > 0:
                    self.assertIn("expiry_time", data,
                                f"expiry_time should be set when TTL={ttl}")
                else:
                    # When TTL is 0, expiry_time should not be set
                    self.assertNotIn("expiry_time", data,
                                   f"expiry_time should NOT be set when TTL={ttl}")
    ...
    def test_no_project_id_specified(self, mock_client_class):
        ...
        self.assertNotIn("project", call_kwargs,
                        "Baseline behavior: project parameter should NOT be passed when project_id is not configured")
