...
class CrewAISession(Storage):
    ...
    def search(self, query: str, limit: int = 10, score_threshold: float = 0.5) -> list[dict[str, Any]]:
        ...
        if limit <= 0:
            return []
        return [{"content": item["value"], "context": item["value"]} for item in self._items[-limit:]]
...
class CrewAIRunner(Runner):
    ...
    def _memory(self, session: Session) -> ExternalMemory | None:
        ...
        return ExternalMemory(storage=previous)
    ...
    async def run(self, agent: Any, session: Session, requests: list[AgentRequest]) -> AgentReply:
        ...
        try:
            ...
            # Persist the user's prompt so external memory search can retrieve it on subsequent turns.
