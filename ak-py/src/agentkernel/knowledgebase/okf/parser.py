...
This module is deliberately store-free and network-free. It takes a bundle-relative path
and the document text, and returns objects. That is what lets one OKF reader serve a
bundle from a local directory in development and from an S3 prefix in production without
either the reader or the store changing — and it is asserted by a test that fails if
...
conditions that skip a concept are enumerated on :func:`parse_concept`; every other
surprise is carried.
...
def decode_document(data: bytes) -> str:
    ...
    ``errors="replace"`` is load-bearing rather than defensive: the manifest walk reads a
    bounded *prefix* of each file, which can cut a multi-byte character in half. A strict
    decode would abort the walk over one non-ASCII concept.
...
def derive_trust(verified: list[dict[str, Any]]) -> TrustTier:
    ...
    Not from ``generated``, not from ``status``, not from staleness. The tier is an advisory
    signal that rides on every record; it is never grounds for filtering one out.
...
def is_stale(stale_after: str | None, now: datetime) -> tuple[bool, list[OKFDiagnostic]]:
    ...
    ``now`` is injected rather than read from the clock so the answer is deterministic under
    test. A naive deadline is read as UTC, since OKF timestamps are ISO-8601 instants.
    ...
    try:
        ...
        return False, [_diagnostic("", DiagnosticCode.UNPARSEABLE_STALE_AFTER, message)]
...
def extract_links(concept_path: str, body: str) -> tuple[list[str], list[OKFDiagnostic]]:
    ...
    bundle-absolute (``/tables/customers.md``) or relative (``./other.md``). Link semantics
    live in the surrounding prose.
    ...
    for match in _LINK.finditer(body or ""):
        ...
        if resolved == ".." or resolved.startswith("../"):
            diagnostics.append(_diagnostic(concept_path, DiagnosticCode.PATH_ESCAPE, f"link escapes the bundle namespace: {target!r}"))
            continue
...
def parse_concept(path: str, data: str, *, body_complete: bool, now: datetime | None = None) -> tuple[OKFConcept | None, list[OKFDiagnostic]]:
    ...
    id list the fetch tool takes; frontmatter that is absent, unparseable, or not a mapping;
    and a missing or empty ``type``, the one key OKF requires. Everything else is carried:
    unknown keys reach ``extra`` untouched, an unknown ``type`` value is kept verbatim, and
    a scalar where a collection was expected is normalised with a diagnostic.
    ...
    if "," in path:
        ...
        return None, [_diagnostic(path, DiagnosticCode.COMMA_IN_PATH, message)]

    frontmatter_text, body = split_frontmatter(data)
    if frontmatter_text is None:
        return None, [_diagnostic(path, DiagnosticCode.UNPARSEABLE_FRONTMATTER, "document has no complete '---' frontmatter block")]

    try:
        loaded = yaml.safe_load(frontmatter_text)
    except yaml.YAMLError as error:
        return None, [_diagnostic(path, DiagnosticCode.UNPARSEABLE_FRONTMATTER, f"frontmatter is not valid YAML: {error}")]

    if not isinstance(loaded, dict):
        return None, [_diagnostic(path, DiagnosticCode.UNPARSEABLE_FRONTMATTER, f"frontmatter is not a mapping: {type(loaded).__name__}")]

    concept_type = loaded.get("type")
    if not isinstance(concept_type, str) or not concept_type.strip():
        return None, [_diagnostic(path, DiagnosticCode.MISSING_TYPE, f"'type' is required and must be a non-empty string, got {concept_type!r}")]

    diagnostics: list[OKFDiagnostic] = []
    tags = _as_tags(path, loaded.get("tags"), diagnostics)
    verified = _as_verified(path, loaded.get("verified"), diagnostics)

    stale, stale_diagnostics = is_stale(_as_optional_str(loaded.get("stale_after")), now or datetime.now(timezone.utc))
    ...
    links: list[str] = []
    if body_complete:
        links, link_diagnostics = extract_links(path, body)
    ...
    try:
        concept = OKFConcept(
            path=path,
            type=concept_type,
            title=_as_optional_str(loaded.get("title")),
            description=_as_optional_str(loaded.get("description")),
            resource=_as_optional_str(loaded.get("resource")),
            tags=tags,
            status=_as_optional_str(loaded.get("status")),
            stale_after=_as_optional_str(loaded.get("stale_after")),
            generated=_as_mapping(loaded.get("generated")),
            verified=verified,
            sources=_as_mapping_list(loaded.get("sources")),
        ...
        return None, [_diagnostic(path, DiagnosticCode.UNPARSEABLE_FRONTMATTER, f"frontmatter could not be modelled: {error}")]
...
def parse_index(path: str, data: str, *, is_root: bool) -> tuple[str, str | None, list[OKFDiagnostic]]:
    ...
    An index carries no frontmatter, except at the bundle root where ``okf_version`` is the
    single permitted key. Other keys there are unrecognised content, which tolerance says to
    carry rather than reject, so they draw no diagnostic. An index elsewhere carrying
    frontmatter draws one — and its body is still used, because rejecting a curated listing
    over a stray block would lose more than it protects.
    ...
    :return: The body to serve as the curated listing, the declared ``okf_version`` if any,
        and any diagnostics.
    """
    frontmatter_text, body = split_frontmatter(data)
    if frontmatter_text is None:
    ...
    if not is_root:
        ...
        return body, None, [_diagnostic(path, DiagnosticCode.INDEX_FRONTMATTER, message)]

    try:
        loaded = yaml.safe_load(frontmatter_text)
    except yaml.YAMLError as error:
        return body, None, [_diagnostic(path, DiagnosticCode.UNPARSEABLE_FRONTMATTER, f"index frontmatter is not valid YAML: {error}")]
    ...
    return body, version, [_diagnostic(path, DiagnosticCode.VERSION_MISMATCH, message)]
...
def _as_mapping_list(value: Any) -> list[dict[str, Any]]:
    ...
    if not isinstance(value, list):
        return []
    return [_as_mapping(entry) for entry in value if isinstance(entry, dict)]
...
def _as_verified(path: str, value: Any, diagnostics: list[OKFDiagnostic]) -> list[dict[str, Any]]:
    ...
    if isinstance(value, dict):
        diagnostics.append(_diagnostic(path, DiagnosticCode.COERCED_SCALAR, "'verified' was a bare mapping; read as a one-element list"))
        return [_as_mapping(value)]
    return _as_mapping_list(value)
...
def _as_tags(path: str, value: Any, diagnostics: list[OKFDiagnostic]) -> list[str]:
    ...
    if isinstance(value, list):
        if any(not isinstance(entry, str) for entry in value):
            diagnostics.append(_diagnostic(path, DiagnosticCode.COERCED_SCALAR, "a non-string 'tags' entry was stringified"))
        return [str(entry) for entry in value]

    diagnostics.append(_diagnostic(path, DiagnosticCode.COERCED_SCALAR, f"'tags' was a scalar {value!r}; read as a one-element list"))
    return [str(value)]
