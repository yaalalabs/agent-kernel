...
from walledai import WalledRedact, WalledProtect
from .guardrail import InputGuardrail, OutputGuardrail, BaseGuardrailUtil
...
class WalledAIInputGuardrail(InputGuardrail, WalledAIGuardrailBase, BaseGuardrailUtil):
    ...
    async def on_run(self, session: Session, agent: Agent, requests: list[AgentRequest]) -> list[AgentRequest] | AgentReply:
        ...
        safety_res = await to_thread(
            silent_call, 
            self.protect_client.guard, 
            raw_text, 
        ...
        redact_res = await to_thread(
            silent_call, 
            self.redact_client.guard, 
            raw_text
        )
        ...
        if isinstance(redact_res, dict) and 'data' in redact_res:
            ...
            log.info(f'Redacted input text: {masked_text}')
            log.info(f'mapping: {existing_mapping}')
