"""Facebook Messenger inbound/outbound adapter pair (spec #524 §9)."""
...
_log = logging.getLogger("ak.integration.messenger")


def _client() -> MetaSendAPIClient:
    config = AKConfig.get().messenger
    if not all([config.access_token, config.verify_token]):
        _log.error("Facebook Messenger configuration is incomplete. Please set access_token and verify_token.")
        raise ValueError("Incomplete Facebook Messenger configuration.")
    return MetaSendAPIClient(
        base_url=f"https://graph.facebook.com/{config.api_version or 'v24.0'}",
        access_token=config.access_token,
        message_fields={"messaging_type": "RESPONSE"},
        log=_log,
    )


class MessengerInboundAdapter(InboundAdapter):
    """Messenger webhook events -> normalized requests."""
    ...
    def __init__(self):
        config = AKConfig.get()
        self._agent = config.messenger.agent or None
        self._max_file_size = config.api.max_file_size
        self._app_secret = config.messenger.app_secret
        self._verify_token = config.messenger.verify_token
        self._api = _client()
    ...
    async def _to_request(self, event: dict) -> Optional[InboundRequest]:
        ...
        if "delivery" in event:
            self._log.debug(f"Message delivery confirmation: {event['delivery']}")
        elif "read" in event:
    ...
    async def _build(self, sender_id: str, request_id: str, text: str, attachments: list) -> Optional[InboundRequest]:
        ...
        return InboundRequest(
            # Messenger has no thread of its own: the sender is the conversation.
    ...
    async def _add_attachment(self, attachment: dict, requests: List[AgentRequest]) -> None:
        ...
        try:
            ...
            requests.append(AgentRequestText(prompt="I couldn't process one of your attachments. Please try again or send it in a different format."))


class MessengerOutboundAdapter(OutboundAdapter):
    """Agent replies -> Messenger messages."""
    ...
    async def deliver_error(self, message: str, reply_context: Dict[str, str]) -> None:
        ...
        try:
            ...
            self._log.error(f"Could not deliver the Messenger error message: {e}")
