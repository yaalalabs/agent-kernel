...
class RedisResponseStore(ChunkStreamMixin, ResponseStore):
    """Redis-backed response store: the record mailbox, plus per-request chunk streaming
    ...
    def get_record(self, request_id: str, get_and_delete: bool = False) -> dict | None:
        self._log.debug("Getting Redis response record for request_id=%s get_and_delete=%s", request_id, get_and_delete)
        raw_message = self._driver.get(self._driver.key(request_id))
        if raw_message is None:
            return None
        message = json.loads(raw_message)
