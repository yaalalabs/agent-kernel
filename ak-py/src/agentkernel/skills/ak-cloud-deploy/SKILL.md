...
Use current module version (`0.5.0`) unless user requests another.
...
```toml
dependencies = [
  "agentkernel[openai,api,aws]>=0.5.0"  # include 'redis' if using Redis session/response store
]
```
...
```toml
dependencies = [
  "agentkernel[openai,api,aws,redis,auth]>=0.5.0"
]
```
...
```toml
dependencies = [
  "agentkernel[openai,api,gcp]>=0.5.0"      # for Firestore sessions
  # or: "agentkernel[openai,api,redis]>=0.5.0"  # for Redis sessions
]
```
