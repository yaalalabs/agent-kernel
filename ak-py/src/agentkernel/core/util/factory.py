...
def resolve_dotted(path: str, *, base: type[T]) -> type[T]:
    ...
    un-importable, missing attribute, or not a ``base`` subclass — raises ``AKConfigError``,
    since a user-supplied path that does not resolve is a configuration error, not a crash.
    """
    module_path, _, attr = path.rpartition(".")
    if not module_path:
        raise AKConfigError(f"'{path}' is not a dotted path to a {base.__name__} subclass")
    try:
        obj = getattr(importlib.import_module(module_path), attr)
    except (ImportError, AttributeError) as exc:
        raise AKConfigError(f"could not import '{path}': {exc}") from exc
    if not (isinstance(obj, type) and issubclass(obj, base)):
        raise AKConfigError(f"'{path}' is not a {base.__name__} subclass")
    return obj
