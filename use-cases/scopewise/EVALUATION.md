...
- Automated unit/API checks cover ownership, CSRF, file rejection, bounded body bytes, source quote validation, objective-specific citations, the three-file upload-to-review preparation job, exclusion-first candidate selection, local-embedding fallback, assistant rejection of unapproved evidence, bounded Agent Kernel provenance, change-impact history, explicit exclusions, partial coverage, duplicate removal, changing lecturers/syllabuses, no-op suppression, changing a reviewed judgment, stale exports, failed/interrupted model jobs, manual review, Telegram linking/secrets/replay, UI contracts, judge checks, and a temporary SQLite backup/restore.
...
- The final Compose definition resolves with the documented local environment file. `pip-audit` reported no known vulnerabilities in the fully pinned Python 3.12 dependency set on 2026-09-01. The container operating-system layers were not sent to a remote scanner and still require a local image scan before public launch.
...
**Do not rely on automatic judgments from the current Llama configuration.** Use the evidence review controls to correct suggestions. Manual review can be started without a model and is labeled as such. Human confirmation is required before source judgments enter a pack. Optional generated questions require explicit opt-in, remain labeled for inspection and must not be treated as predictions. The quote validator proves a quote exists at a source location; it cannot prove that the model interpreted the quote or generated question correctly.
