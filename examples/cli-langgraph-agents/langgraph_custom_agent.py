...
Only use handoffs when the task genuinely requires the other agent's specialized capabilities."""
    ...
    def _llm_call(self, state: AgentState) -> dict:
        ...
        self._log(f"Invoking LLM {'with' if use_handoff else 'without'} handoff... Input State: {state}")
        response: BaseMessage = self.model.invoke(messages)
        self._log(f"LLM Response {'with' if use_handoff else 'without'} handoff... Response Type: {response.type}, Response: {response}")
        ...
        if use_handoff and response.content:
            try:
                ...
                self._log(f"Parsed LLM response with handoff check: {response_dict}", force=True)
                return {
                    "messages": [AIMessage(
                        content=structured_response.llm_response_message,
                ...
                self._log(f"Error parsing LLM response therefore returning raw response. ERROR: {e}", force=True)
        
        self._log("Returning raw response without handoff processing", force=True)
        return {"messages": [response]}

    def _select_next_node(self, state: AgentState) -> str:
        handoff = state.get("handoff_to_agent")
        self._log(f"Selecting next node, handoff requested: {handoff}", force=True)
        if handoff and handoff in self.handoff_agents:
            self._log(f"Valid handoff to agent: {handoff}")
            return handoff
        elif handoff:
            self._log(f"WARNING: Invalid handoff requested to '{handoff}', available agents: {list(self.handoff_agents.keys())}", force=True)
    ...
    def _create_handoff_node(self, target_agent: Union['LangGraphAgent', CompiledStateGraph]):
        ...
        def handoff_node(state: AgentState) -> dict:
            ...
            self._log(f"Executing handoff to agent: {target_agent.name}", force=True)
            result = target_agent.invoke(state_for_handoff_node)["messages"][-1]
            self._log(f"Handoff agent '{target_agent.name}' Execution Completed", force=True)
            self._log(f"Result from handoff agent '{target_agent.name}': {result}")
    ...
    def _build_graph(self) -> CompiledStateGraph:
        ...
        self._log(f"Adding main LLM call node: {self.name}")
        graph.add_node(self.name, self._llm_call)

        if self.tool_node:
            self._log("Adding tools node")
            graph.add_node("tools", self.tool_node)

        for agent_name, agent in self.handoff_agents.items():
            self._log(f"Adding handoff node for agent: {agent_name}")
            graph.add_node(agent_name, self._create_handoff_node(agent))

        edge_mapping = {"__end__": END}
        if self.tool_node:
            self._log("Adding tools to edge mapping")
            edge_mapping["tools"] = "tools"

        for agent_name in self.handoff_agents.keys():
            self._log(f"Adding '{agent_name}' to edge mapping")
            edge_mapping[agent_name] = agent_name

        self._log(f"Adding conditional edges from main node EDGE MAPPING: {edge_mapping}")
        graph.add_conditional_edges(self.name, self._select_next_node, edge_mapping)

        if self.tool_node:
            self._log(f"Adding edge from tools back to main node")
            graph.add_edge("tools", self.name)

        for agent_name in self.handoff_agents.keys():
            self._log(f"Adding edge from handoff agent '{agent_name}' to END")
            graph.add_edge(agent_name, END)

        self._log(f"Setting entry point: {self.name}")
        graph.set_entry_point(self.name)

        compiled = graph.compile(name=self.name, checkpointer=self.checkpointer)
        self._log(f"Graph compilation complete")
        return compiled
