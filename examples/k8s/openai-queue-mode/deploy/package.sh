...
#   nats|kafka   which config variant to bake in as config.yaml (default: nats)
#
# Mirrors examples/aws-containerized/openai-stream-queue-mode/deploy/deploy.sh: dependencies
# are staged into dist-*/data with pip's --target layout and copied into python:3.12-slim.
# The images run on Linux whatever the build host is, so uv cross-installs for the Linux
# platform matching the host architecture (Docker builds target it by default).
...
if [[ $TRANSPORT != "nats" && $TRANSPORT != "kafka" ]]; then
  echo "usage: ./package.sh [local] [nats|kafka]" >&2
  exit 2
fi

# manylinux_2_28 because confluent-kafka ships no older-tagged wheels; python:3.12-slim
# (Debian bookworm, glibc 2.36) satisfies it comfortably.
case "$(uname -m)" in
...
create_deployment_package() {
  ...
  cp "$entry" "$dist/data/"
  cp "config.${TRANSPORT}.yaml" "$dist/data/config.yaml"

  popd

  cp "Dockerfile.${component}" "../${dist}/Dockerfile"
  docker build -t "ak-example-${component}:${IMAGE_TAG}" "../${dist}"
}
