...
async def test_disclaimer_hook_adds_disclaimer(http_client):
    ...
    assert "Disclaimer" in response or "disclaimer" in response.lower(), f"Expected disclaimer in response, got: {response}"
    assert "generated by an AI assistant and should be verified for accuracy" in response or "generated by an ai assistant and should be verified for accuracy" in response.lower(), f"Expected AI disclaimer text, got: {response}"
...
async def test_disclaimer_hook_with_guard_rail_rejection(http_client):
    ...
    assert "cannot assist" in response.lower() or "apologize" in response.lower(), f"Expected guard rail rejection, got: {response}"
