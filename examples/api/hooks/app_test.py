...
This test validates that both the GuardRail and RAG hooks work correctly,
both individually and when chained together.
"""
...
async def test_guard_rail_blocks_inappropriate_request(http_client):
    ...
    assert "cannot assist" in response.lower() or "apologize" in response.lower(), \
        f"Expected guard rail rejection, got: {response}"
    assert "hack" in response.lower(), \
        f"Expected rejection message to mention 'hack', got: {response}"
...
async def test_guard_rail_allows_safe_request(http_client):
    ...
    assert "cannot assist" not in response.lower(), \
        f"GuardRail should not block this request, got: {response}"
    assert "Paris" in response or "paris" in response.lower(), \
        f"Expected answer about Paris, got: {response}"
...
async def test_rag_hook_injects_context(http_client):
    ...
    assert any(keyword in response for keyword in ["framework", "agent", "kernel", "python"]), \
...
async def test_rag_hook_with_hooks_topic(http_client):
    ...
    assert any(keyword in response.lower() for keyword in ["pre", "post", "execution", "callback"]), \
...
async def test_hooks_chaining_rag_then_guard_rail(http_client):
    ...
    assert any(keyword in response1.lower() for keyword in ["programming", "language", "python"]), \
    ...
    assert "cannot assist" in response2.lower() or "apologize" in response2.lower(), \
...
async def test_long_input_guard_rail(http_client):
    ...
    assert "too long" in response.lower(), \
        f"Expected guard rail to block long input, got: {response}"
...
async def test_no_rag_context_available(http_client):
    ...
    assert "cannot assist" not in response.lower(), \
        f"Should not be blocked by guard rail, got: {response}"
