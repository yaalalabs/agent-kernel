...
module "serverless_agents" {
  ...
  function_name        = "multimodal-redis"
  handler_path         = "lambda.handler"
  module_name          = var.module_name
  package_path         = "../dist.zip"
  memory_size          = 256
  create_redis_cluster = false # Set to true to provision a new cluster, or configure the URL in config.yaml
  product_display_name = "Agent Kernel Multimodal Redis"
