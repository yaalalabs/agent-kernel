...
1. Starts the Agent Kernel server with Messenger integration
2. Sends a realistic HMAC-signed Messenger webhook payload
3. Verifies the webhook is processed successfully
4. Checks that the agent generates the correct response

Required environment variables:
    AK_MESSENGER__ACCESS_TOKEN - Page access token for the server
    AK_MESSENGER__VERIFY_TOKEN - For webhook verification
    AK_MESSENGER__APP_SECRET   - For signing webhook requests (optional but recommended)
    AK_TEST_MESSENGER_USER_ID  - Test user's Page-Scoped ID (PSID)
    OPENAI_API_KEY             - For the AI agent

Usage:
    cd examples/api/messenger && ./build.sh local
...
ACCESS_TOKEN = os.environ.get("AK_MESSENGER__ACCESS_TOKEN", "")
VERIFY_TOKEN = os.environ.get("AK_MESSENGER__VERIFY_TOKEN", "")
APP_SECRET = os.environ.get("AK_MESSENGER__APP_SECRET", "")
TEST_USER_ID = os.environ.get("AK_TEST_MESSENGER_USER_ID", "")
PAGE_ID = os.environ.get("AK_MESSENGER__PAGE_ID", "")
OPENAI_KEY = os.environ.get("OPENAI_API_KEY", "")
API_VERSION = "v24.0"

SKIP_REASON = "Missing required env vars for Messenger integration test"
...
async def _get_conversation_id() -> str:
    ...
    params = {"platform": "messenger", "user_id": TEST_USER_ID, "access_token": ACCESS_TOKEN}
    async with httpx.AsyncClient(timeout=10.0) as client:
...
async def _get_latest_message_from_conversation(conversation_id: str) -> dict:
    ...
    # Get message IDs
    url = f"https://graph.facebook.com/{API_VERSION}/{conversation_id}"
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        url = f"https://graph.facebook.com/{API_VERSION}/{latest_msg_id}"
...
async def _send_message_to_user(user_id: str, text: str) -> str:
    ...
    payload = {
        ...
        "tag": "CONFIRMED_EVENT_UPDATE",  # Required for sending outside 24h window in test scenarios
    }
...
def _compute_messenger_signature(body: str) -> str:
    """Compute a valid Messenger request signature using HMAC-SHA256."""
    if not APP_SECRET:
        return ""
    signature = hmac.new(
...
def _build_messenger_event_payload(sender_id: str, text: str) -> dict:
    """Build a realistic Messenger 'message' event payload."""
...
@pytest.mark.asyncio
@pytest.mark.skipif(SHOULD_SKIP, reason=SKIP_REASON)
async def test_messenger_integration(server_process):
    """
    End-to-end Messenger integration test.
    ...
    This test now works just like Slack/Gmail tests - with proper validation!
    """
    # Send the question to the user in Messenger first (so they see it in their app)
    print(f"\n Sending question to Messenger: '{TEST_QUESTION}'")
    try:
        await _send_message_to_user(TEST_USER_ID, f"🧪 Integration Test Question:\n{TEST_QUESTION}")
        print("Question sent to Messenger")
    except Exception as e:
        print(f"Could not send question to Messenger: {e}")

    # Wait a moment for the message to be delivered
    await asyncio.sleep(2)

    # Build and send the signed webhook (to trigger bot processing)
    payload = _build_messenger_event_payload(TEST_USER_ID, TEST_QUESTION)
    body_str = json.dumps(payload)
    signature = _compute_messenger_signature(body_str)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await client.post(
            f"{SERVER_URL}/messenger/webhook",
            content=body_str,
            headers=headers,
        )
    ...
    print("\n Reading bot's response from Messenger...")
    ...
    # Validate the response using Test.compare (like Slack/Gmail tests)
    ...
    # Send the result to Messenger (like Slack test does)
    print("\n Sending test result to Messenger...")
    if test_passed:
    ...
    try:
        ...
        print(" Result sent to Messenger")
    except Exception as e:
        print(f" Could not send result to Messenger: {e}")

    if test_passed:
        print("✅MESSENGER INTEGRATION TEST PASSED")
    else:
        print("❌ MESSENGER INTEGRATION TEST FAILED")
