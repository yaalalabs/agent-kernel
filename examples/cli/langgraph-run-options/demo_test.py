...
async def test_a_tool_turn_reports_progress_and_the_declared_limit(test_client):
    ...
    assert stats["recursion_limit"] == 50  # the declared config value, verbatim
    assert stats["llm_calls"] >= 1  # on_chat_model_start fired
    assert stats["tool_calls"] >= 1  # on_tool_start fired
...
async def test_counters_are_per_turn(test_client):
    ...
    stats = _stats(response)
    assert stats["recursion_limit"] == 50
