...
async def test_a_tool_turn_reports_progress_and_the_declared_limit(test_client):
    ...
    assert stats["request_limit"] == 10  # the declared UsageLimits value, verbatim
    assert stats["llm_calls"] >= 1  # ctx.usage.requests read in the event_stream_handler
    assert stats["tool_calls"] >= 1  # a FunctionToolCallEvent was seen
