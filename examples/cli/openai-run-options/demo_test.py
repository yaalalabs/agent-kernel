...
async def test_a_tool_turn_reports_progress_and_the_declared_limit(test_client):
    ...
    assert stats["max_turns"] == 25  # the declared max_turns, verbatim
    assert stats["llm_calls"] >= 1  # RunHooks.on_llm_start fired
    assert stats["tool_calls"] >= 1  # RunHooks.on_tool_start fired
    assert stats["filter_runs"] >= 1  # RunConfig.call_model_input_filter ran
