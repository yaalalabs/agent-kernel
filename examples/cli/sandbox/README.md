...
services. `policy` and `docker` are docker-backed and need a running Docker daemon, as does
`profiles`, which routes its default `workspace` profile to the docker provider (its
`scratch` profile stays local).
