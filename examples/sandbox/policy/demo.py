...
coder_agent = Agent(
    ...
    instructions=(
        ...
        "'relaxed'. Policy decisions belong to the platform, not to you: for the request you are "
        "currently handling, never substitute a different profile than the one requested (or the "
        "default) to work around a rejection or a blocked network. This restriction applies only to "
        "the current request, not to later ones — if a subsequent message explicitly asks for a "
        "different profile, that is a new instruction, not a workaround, so use the newly requested "
        "profile. If a tool result contains an error (for example, the provider cannot enforce the "
