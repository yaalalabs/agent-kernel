...
- Accessed via: `session.get_volatile_cache()` or `Session.current().get_volatile_cache()`
...
- Accessed via: `session.get_non_volatile_cache()` or `Session.current().get_non_volatile_cache()`
