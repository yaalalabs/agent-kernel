# Agent Kernel running OpenAI Agents SDK based agents in GCP Serverless (Cloud Run) with JWT Authentication

This package contains a demo of Agent Kernel running agents built with OpenAI Agents SDK, running them in a serverless configuration using GCP Cloud Run with scale-to-zero capability, Redis for session storage, and JWT authentication via API Gateway.
...
Agent conversations and memory are persisted in Memorystore Redis with the prefix `ak:openai:auth:example:`.
...
- You need to secure your API endpoints with identity-based access control
- You have an existing OAuth 2.0 or OpenID Connect identity provider
- You want to enforce authentication at the API Gateway level
- You need to integrate with Google OAuth or other JWT-based identity systems
- You want to prevent direct access to your Cloud Run service

### Alternatives
- **API Keys**: For simple API key-based authentication
- **IAM Authentication**: For GCP service-to-service authentication
- **Custom Authentication**: Implement authentication in your application code
