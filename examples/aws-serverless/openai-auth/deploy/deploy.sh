...
create_deployment_package() {
    ...
    if [[ ${1-} != "local" ]]; then
      ...
      uv pip install --force-reinstall --no-deps --no-index --target=dist/data --find-links ../../../ak-py/dist agentkernel[openai,redis,auth] --no-cache-dir
    fi
...
create_auth_deployment_package() {
    ...
    if [[ ${1-} != "local" ]]; then
        ...
        uv pip install --force-reinstall --no-deps --no-index agentkernel[api,aws,auth] --target=dist_auth --find-links ../../../ak-py/dist --no-cache-dir
    fi
