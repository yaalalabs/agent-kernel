...
#   protocols           = ["https"]
#   service_url         = "https://${azurerm_linux_function_app.function.default_hostname}"
...
# # Local variables for path processing
# locals {
#   # Process endpoints or use default
#   endpoints = length(var.gateway_endpoints) > 0 ? var.gateway_endpoints : [
#     {
...
#       path_parts = split("/", trimprefix(ep.path, "/"))
#     }
#   ]
...
#     "${ep.method}-${replace(ep.full_path, "/", "-")}" => ep
#   }
# }
...
#   api_management_name = azurerm_api_management.apim.name
#   protocol            = "http"
#   url                 = "https://${azurerm_linux_function_app.function.default_hostname}/api"
...
#   api_management_name = azurerm_api_management.apim.name
#   resource_group_name = data.azurerm_resource_group.rg.name
#   display_name        = "${each.value.method} ${each.value.full_path}"
#   method              = each.value.method
#   url_template        = "/${var.api_version}/${each.value.full_path}"
#   description         = "Endpoint for ${each.value.method} ${each.value.full_path}"
...
# # Policy to route to Function App
...
#     <rewrite-uri template="/${each.value.full_path}" />
#   </inbound>
#   <backend>
#     <base />
#   </backend>
...
resource "azurerm_api_management_api" "rest_api" {
  ...
  path                = var.api_base_path != null && var.api_base_path != "" ? var.api_base_path : ""
  protocols           = ["https"]
  service_url         = "https://${azurerm_function_app_flex_consumption.function.default_hostname}"
