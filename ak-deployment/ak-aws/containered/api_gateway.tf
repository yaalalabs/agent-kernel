...
resource "aws_api_gateway_vpc_link" "ecs_alb" {
  name = "${var.product_alias}-${var.env_alias}-apigw-vpclink"
...
resource "aws_api_gateway_integration" "agent_endpoint_post" {
  ...
  request_parameters = { "integration.request.header.Host" = aws_lb.app.dns_name }
  type                    = "HTTP_PROXY"
  integration_http_method = "ANY"
  uri                     = "http://${aws_lb.app.dns_name}:${aws_lb_listener.http.port}/api/${var.api_version}/${var.agent_endpoint}"
  connection_type         = "VPC_LINK"
  connection_id           = aws_api_gateway_vpc_link.ecs_alb.id
}
