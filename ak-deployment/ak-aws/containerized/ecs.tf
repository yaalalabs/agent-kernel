    ...
    (local.service_name) = {
      ...
      container_definitions = {
        (local.container_name) = {
          ...
          health_check = {
            command = ["CMD-SHELL", "curl -sf http://localhost:${var.ecs_container_port}${local.api_base_segment_with_version}${var.ecs_health_check_endpoint} || exit 1"],
            interval    = 30,
            timeout     = 5,
            retries     = 3,
            startPeriod = 10
...
resource "aws_lb_target_group" "app" {
  ...
  target_type = "ip"
  health_check {
    path                = "${local.api_base_segment_with_version}${var.ecs_health_check_endpoint}"
    healthy_threshold   = 2
