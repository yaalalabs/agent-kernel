...
| `envelope.py` | `QueueMessage` plus the attribute constants: `REQUEST_ID`, `USER_ID`, `ENDPOINT_URL`, `STATUS_CODE`, and (#524) `INTEGRATION` — whose presence marks integration traffic and whose value resolves the outbound adapter — and `REPLY_CONTEXT_PREFIX` (`reply_`) |
...
1. **Activation**: `RESTAPI.run()` delegates to `IOHandler` only when **all three** hold: `cls is
   ...
   messaging-integration webhook host does, because a queue producer on a bare FastAPI app would
   enqueue into a queue no runner drains and the caller would still get its 200 (#524).
