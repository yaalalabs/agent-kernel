...
- **Mounting**: explicit on any surface — `RESTAPI.run(handlers=[AgentRESTRequestHandler(), ScheduleRESTRequestHandler(authoriser=...)])`. The pipeline single-process topology mounts it for you: `IOHandler.run(authoriser=...)` appends the handler whenever `AKConfig.get().schedule` is present, and calls `ScheduleManager.get()` eagerly at startup so an unusable provider/transport pairing or incomplete provider config fails the boot instead of the first request. `RESTAPI.run()`'s delegation rule is untouched, so an app needing an `Authoriser` in this topology calls `IOHandler.run(authoriser=...)` directly (the parameter is named generically: it protects whatever management routes the topology mounts, not the schedule routes specifically)
