...
- **Mounting**: always the application's, on every surface, exactly like the Slack and thread handlers — `RESTAPI.run(handlers=[AgentRESTRequestHandler(), ScheduleRESTRequestHandler(authoriser=...)])`, or `IOHandler.run(handlers=[ScheduleRESTRequestHandler(authoriser=...)])` in the pipeline single-process topology, where the passed handlers are mounted **alongside** the pipeline's own `RequestHandler()` (the queue producer is not a replaceable default). Nothing is mounted from config: a `schedule` block enables deferring and the agent tools, not routes. The boot fail-fast is independent of mounting — `IOHandler._validate_topology` calls `ScheduleManager.get()` whenever `config.schedule` is present, so an unusable provider/transport pairing or incomplete provider config fails the boot instead of the first request, including for an agent-tools-only app. `RESTAPI.run()`'s delegation rule is untouched (no-explicit-handlers only), so an app wanting the routes in this topology calls `IOHandler.run(handlers=[...])` directly
