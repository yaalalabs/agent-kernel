...
2. **Coupling**: `pipeline` imports `core` and `api` only (api's pipeline imports are lazy inside
   ...
   which is how `IOHandler` types its `pollers` parameter). The one exception is
   `ResponseHandler._outbound_adapter`, which imports `IntegrationAdapterFactory` lazily *inside
   the method*: messaging platforms are an `integration` capability, and a module-scope import
   would make every pipeline process pay for their SDKs. Same shape as `core/tool.py` reaching
   the AG-UI state helpers.
