...
| `test_knowledgebase_okf_tools.py` | The OKF agent surface end to end over real bundles in `tmp_path`: the tool set per role, `func.__name__ == SystemTool.name`, read scoping falling out of the per-agent builder, the per-`(agent, database)` write refusal as a string, the prompt riding the first tool's description, `SystemToolFactory` wiring, and write permission keyed to the tool's owner across a handoff (a stale entry-agent `ToolContext` is ignored) |
