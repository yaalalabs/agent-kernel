...
| `test_knowledgebase_okf_tools.py` | The OKF agent surface end to end over real bundles in `tmp_path`: the tool set per role, `func.__name__ == SystemTool.name`, read scoping falling out of the per-agent builder, the per-`(agent, database)` write refusal as a string, the prompt riding the first tool's description, `SystemToolFactory` wiring, and `_calling_agent()`'s fallbacks |
