...
- **`copilot-review-request.yaml`**: Requests a Copilot code review on open/reopen/ready-for-review using the `COPILOT_REQUEST_TOKEN` PAT (`pull_request_target`, API-only, no checkout). Exists because the develop ruleset's `copilot_code_review` rule only fires for authors who hold a Copilot license. Skips PRs where Copilot is already requested and bot-authored PRs; `workflow_dispatch` takes a PR number for backfills
