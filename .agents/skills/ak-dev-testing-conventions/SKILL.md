...
| `test_pipeline_io_handler.py` | `IOHandler.run()` topology validation and fail-fasts (ASYNC-on-`in_memory` without a validator, broker WS modes without a push token, broker + non-shared response store, configured scheduling built at boot vs unusable scheduling config failing it), schedule routes mounted by the application rather than from config, signal handlers, graceful-drain exit code |
