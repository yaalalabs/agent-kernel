...
| `test_trace_langfuse_langgraph.py` | LangFuse LangGraph trace runner; a caller-supplied `callbacks` list is appended after the Langfuse handler rather than replacing it (#754); a factory-merged `config` reaching `ainvoke` through the traced runner with the Langfuse handler kept first (#758) |
