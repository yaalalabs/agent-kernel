...
- Pipeline single-process topology: `IOHandler.run` gains `authoriser: Optional[Authoriser] = None` — named generically, since it protects whatever resource-management routes this topology mounts on the application's behalf, not the schedule routes specifically — and builds `handlers=[RequestHandler()]` plus (lazy import) `ScheduleRESTRequestHandler(authoriser=authoriser)` when `AKConfig.get().schedule is not None` (`pipeline/io_handler.py:52`). `IOHandler.run` also calls `ScheduleManager.get()` eagerly at startup so provider/transport incompatibility and missing provider config fail the boot, not the first request. `RESTAPI.run()`'s delegation rule is untouched (`api/http.py`, no-explicit-handlers rule); an app needing an `Authoriser` in this topology calls `IOHandler.run(authoriser=...)` directly.
