...
- **Local-provider single-process constraint** (a second fail-fast at the same construction point):
  ...
  - Anchored on the `local` short name rather than a declared capability, because that short name is what pins the timers to one process.
- **In-memory-store single-process constraint** (a third fail-fast at the same construction point, landing with the eventbridge provider and the distributed stores):
  - `ScheduleManager` raises `AKConfigError` when `schedule.store.type` is `in_memory` and the resolved transport is not `in_memory`. A durable provider manages fine across processes, but the records do not travel: every creation path lands in the runner and every management path in IOHandler, so a listing would not see runner-created tasks and an amendment would report success against a record the firing process never had.
  - Kept separate from the local-provider guard because it constrains the store, not the timers: `eventbridge` + `in_memory` fires correctly and is only unmanageable.
...
- Startup fail-fast, independent of mounting: whenever a `schedule` block is present, `IOHandler` builds `ScheduleManager` while validating the topology, so an unusable provider/transport pairing or an incomplete provider config fails the boot rather than the first request that schedules anything. Tying it to the routes would have exempted the agent-tools-only app, which runs the same backends.
