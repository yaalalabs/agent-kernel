...
- **Provider/transport compatibility** (declared honestly, enforced fail-fast):
  ...
  - `ScheduleManager` validates once at construction (first `get()`): when the declared set is not `None` it must contain `QueueTransportFactory.resolve_type()` (`pipeline/transport/base.py:99-114`, the transport the application declared via `execution.queues.type`, resolved without constructing one so the check works whether or not the pipeline transport class has shipped); mismatch raises `AKConfigError` naming both sides (e.g. "schedule provider 'eventbridge' delivers to SQS, but the configured queue transport is 'in_memory'"). A misconfigured deployment fails at startup or first scheduling use, never at fire time.
