...
| `tests/test_pipeline_io_handler.py` | `pollers` co-hosted as a peer thread on `in_memory`; not started (with a warning) on a broker transport; `request_handler` must be a `RequestHandler` (`TestRequestHandlerValidation`) |
| `tests/test_api_http.py` | `TestPipelineOnlyHandlerRefusal`: `RESTAPI.run` raises `AKConfigError` for a `requires_pipeline` handler passed explicitly, for one a subclass supplies through `get_default_handlers` (the `AWSRestAPI`/`AWSWebsocketAPI` path, which never reaches the `cls is RESTAPI` delegation guard), and for `RequestHandler` itself; an ordinary handler is still served, and the no-handlers delegation path still reaches `IOHandler` unchanged |
