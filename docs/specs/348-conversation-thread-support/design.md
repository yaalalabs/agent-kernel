...
- **Thread naming**:
  ...
  - **When absent**, the name is generated by the active **`ThreadNamingStrategy`** and `name_locked` stays `false`.
    ...
    - **Fallback — thread creation never fails because of naming**: if `litellm` is not installed (it is an optional dependency — install it with the `thread` extra, `pip install "agentkernel[thread]"`), no API key is present, the model call errors, or the model returns an empty reply, the name falls back to a truncated prompt prefix — the first `max_length` characters, trimmed to the last word boundary and suffixed with an ellipsis. The fallback is never silent: a missing `litellm` is warned about once at startup (when the default strategy is constructed) with the install hint, and every failed naming call logs a warning. A blank first prompt skips the LLM call entirely. The fallback is internal behaviour, not a selectable strategy.
