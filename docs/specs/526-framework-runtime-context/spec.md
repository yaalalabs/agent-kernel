...
class Runner(ABC):
    ...
    def _not_picklable(value: Any) -> bool:
        """Whether a single value cannot be pickled — used only to name the offender below."""
        try:
            pickle.dumps(value)
            return False
        except Exception:
...
- **`core/base.py`**: add `FRAMEWORK_CONTEXT` enum member; add `get_framework_context`, `set_framework_context`, `clear_framework_context` to `Session`; add `_load_framework_context`, `_store_framework_context`, `_not_picklable`, `_ensure_framework_context_picklable`, `_log_framework_context_stream_failure` to `Runner`; add `import copy`, `import pickle`, and a module logger (`ak.core.runner`). `Session.__init__`/`get_all`/`clear` and the `Runtime` class are unchanged — in particular `clear()` (`base.py:197-207`) rebuilds `_data` from the two caches only, so it already drops `framework_context`, which is the correct "reset the session" behaviour. `Agent` gains one shared helper, `_append_tools` (change 9 below).
- **`framework/openai/openai.py`**: `OpenAIRunner.run` / `.stream` inject `context=` and write back. `OpenAIAgent.attach_tool` delegates its append loop to `Agent._append_tools` (change 9); `OpenAIModule`/`OpenAIToolBuilder` unchanged.
...
- **`framework/adk/adk.py`**: `GoogleADKRunner._setup_session_context` gains an `injected` param (seeded *under* the AK-internal key) and returns `adk_session`; `GoogleADKSession` gains `get_state()`; `run`/`stream` load, seed, and write back. `State` is added to the existing `google.adk.sessions` import for the scope prefixes. `get_response`'s signature is unchanged but its `run_async` body now drains the stream instead of breaking on the first final response (change 3b below). `GoogleADKAgent.attach_tool` delegates to `Agent._append_tools` (change 9).
...
- **`framework/crewai/crewai.py`**: `CrewAIRunner.run` adds a one-shot warning; no injection, no write-back. `CrewAIAgent.attach_tool` delegates to `Agent._append_tools` (change 9).
...
- **`cli/cli.py`**: `CLI` gains a `_ainput(prompt)` static method and `run()`'s `input(...)` call becomes `await self._ainput(...)`; the unused `AKConfig` import is dropped. Required by the ADK example, not by the feature itself — see change 10 below. No other CLI method changes, and `readline` stays imported so history and line editing behave as before.
- **No changes** to `core/runtime.py`, `core/hooks.py`, `core/session/*`, guardrails, multimodal, sandbox, deployment, or API layers — the key rides the existing `store()`/reload path. `PreHook`/`PostHook` signatures are unchanged; they already receive the `session`, which is all the §1a scope requires. The CLI (`cli/cli.py`) is the one consumer outside this list that changes, for the reason in change 10. (Stale as of a later change: `PreHook.on_run`/`PostHook.on_run`/`PostHook.on_stream_chunk` now receive the framework-native session object as their first parameter (still named `session`, but no longer the AK `Session`) instead of the AK `Session`; `framework_context` access from a hook body goes through `Session.current()`. See `core/hooks.py` and `docs/docs/integrations/hooks.md` for the current contract.)
...
3b. **ADK's `get_response` now drains the event stream instead of breaking on the first final response, so the *last* final response becomes the reply.** Previously it `break`ed out of the `async for` on the first event where `is_final_response()` held and then called `events.aclose()` (`adk.py:180-189` on `develop`). Two consequences, both intentional:
   - **State written after that first final response is now visible.** Breaking early makes ADK cancel its still-running root agent task, so state deltas emitted by later events never reach the session service — which would make `GoogleADKSession.get_state()` read a partial state and silently under-report the round-trip. This is why the change is part of *this* spec rather than a separate one.
   - **Which reply wins changes for multi-agent flows.** `is_final_response()` is true for a sub-agent's final response as well as the root agent's, so the reply is now the root agent's (last) rather than the first sub-agent's. Full drain also removes the early-exit latency saving on runs that answer before the stream ends, and the explicit `aclose()` is dropped as redundant once the generator is exhausted.
   - The synchronous `runner.run(...)` fallback branch keeps its `break` — it has no cancellation problem and no state read-back depends on it.
...
9. **`Agent` gains a shared `_append_tools(agent, wrapped)` helper** (`core/base.py`), and the OpenAI, CrewAI and Google ADK `attach_tool` implementations call it instead of each carrying its own copy of the same loop. No behaviour change: the extracted body is byte-identical in all three (create `tools` when absent or `None`, skip a tool already attached, otherwise append). Justified: `ak-dev-code-patterns` ("Don't duplicate logic") says consolidate at the second copy, and these adapters were already being edited for the context plumbing. Smolagents (dict-or-list collection plus a system-prompt cache rebuild) and Pydantic AI (`FunctionToolset.add_tool`) genuinely differ and are deliberately left alone.

10. **The CLI reads input off the event loop thread** (`cli/cli.py`). `run()`'s blocking `input()` becomes `await self._ainput(...)`, which parks the read on a daemon thread and awaits a future. Motivation: the `adk_context` example (see Documentation updates → Examples) drives Google ADK through `LiteLlm`, and LiteLLM's logging worker is an asyncio task that keeps running between turns — a blocking `input()` on the loop thread froze it, and it timed out its queued callbacks after 20s, printing errors into the middle of the demo. Every CLI example benefits (any framework with background tasks — tracing flushes, provider SDK workers — now makes progress while the prompt waits), but the ADK context demo is what surfaced it, which is why it lands here rather than in a separate PR. The thread is a daemon so `Ctrl+C` exits promptly instead of hanging on a thread parked in `input()`, which is what the default executor would do during `asyncio.run()` shutdown. `readline` is still imported, so history and line editing are unaffected. Covered by `tests/test_cli.py`.

11. **`run()` initializes `prompt = ""` before its `try` in the OpenAI, LangGraph and ADK adapters.** Each adapter's `except Exception` handler returns `AgentReplyText(..., prompt=prompt)`, but `prompt` was only bound by `_process_requests(...)` *inside* the `try`. A request that fails during processing — e.g. `AgentRequestImage(image_data="")` raising `ValueError("no image input provided")` — therefore raised `UnboundLocalError` *from the handler*, which escaped `Runtime.run` as a crash instead of the clean error reply the adapter intends. The smolagents, CrewAI and Pydantic AI adapters already initialized it. Reachable on OpenAI and ADK, whose `_process_requests` raises on malformed image/file input; on LangGraph the guard is defensive only (its `_process_requests` returns `(prompt, False)` rather than raising, so only `ToolContext(...)` construction could get there). Not part of the context feature, but these three `run()` bodies are edited here for the context plumbing and the latent bug sits on the very line that plumbing extends. Regression-tested on the two reachable adapters (`test_openai_runner.py`, `test_adk_runner.py`).
...
- **`ak-py/tests/test_openai_runner.py`**: add cases asserting (a) `MockRunner.run` is called with `context=<the loaded dict>` when the session has `framework_context`; (b) a tool-style in-place mutation of the injected context is written back to the session key after `run()`; (c) on `MockRunner.run` raising, the pre-existing `framework_context` is unchanged. Existing error/structured-output cases unchanged (they pass `context=None`). Plus a regression case for change 11: `AgentRequestImage(image_data="")` returns an `AgentReplyText` starting with `"Error"` and `prompt == ""` instead of raising `UnboundLocalError`.
...
- **`ak-py/tests/test_adk_runner.py`**: `_run_with_response` mocks `_setup_session_context` to a 3-tuple (`:25`) — update to the new 4-tuple `("user", runner_mock, ctx_mock, adk_session_mock)` and give `adk_session_mock.get_state` an `AsyncMock`. Add cases: seeded context is passed into `_setup_session_context`; the `get_state` result minus `ak_tool_context` is written back in full — a seeded key mutated by a tool round-trips **and** a brand-new key a tool added round-trips (asserting the ADK-vs-smolagents divergence); error path leaves the key intact. Also cover the seeding and read-back rules directly: a caller key named `ak_tool_context` does **not** displace the real id in the state delta passed to `update_session_state`, and `GoogleADKSession.get_state()` strips `ak_tool_context` plus `app:`/`user:`/`temp:`-prefixed keys while keeping plain state, looks the session up by the created session's own `app_name`/`user_id`, and returns `{}` when there is no session.
  Because every other case in this file (and in `test_tool_adk.py`) patches `get_response` to an `AsyncMock`, change 3b needs its own direct tests against a fake `run_async` generator: two final responses plus an intermediate event ⇒ the **last** final response is returned **and** the generator was driven to exhaustion (no early `break`); several text parts on one event are space-joined; no final response ⇒ `""` with the stream still drained; a final response whose only part has `text=None` ⇒ `""`. Plus the change 11 regression case, mirroring the OpenAI one.
- **`ak-py/tests/test_langgraph_runner.py`**: `_mock_agent` (`:16`) — add `agent.agent.aget_state` for the stream case. Add cases: `ainvoke` receives `input` containing the spread caller keys plus `messages`; keys present in `result` round-trip, absent ones are dropped; `messages` is never overwritten. For the guarded-failure path below, `aget_state` raising is the realistic source — LangGraph's guarded region is the only one with an extra fallible step.
...
- **`ak-py/tests/test_cli.py`** (new): coverage for change 10 — `readline` is still imported by the module; `_ainput` returns the typed line and passes the prompt through, runs the reader on a named daemon thread, leaves the event loop free to run other tasks while the prompt waits, relays an `input()` exception to the awaiting caller, and discards a line that arrives after cancellation (the resolve-once guard); plus the surrounding `run()` command loop (`!quit`, agent dispatch, blank-line skip, command dispatch and arity/unknown-command errors, agent error recovery, `EOF`, and the no-agent-selected warning).
...
- **`docs/docs/core-concepts/session.md`** — under "How Sessions Work" / "Session Data Storage" (`session.md:358-399`), add a "Framework context / per-run state" subsection: what `framework_context` is, the three accessors (§1a) with a worked pre-hook-seeds / post-hook-reads example, the picklable-`dict` constraint, and the atomic write-back-on-success rule. State the scope explicitly — **hooks use the accessors, tools use the framework-native handle** — and do *not* document `ToolContext.get().session.get_framework_context()` as a supported path. Link out to the hooks page's subsection so the scope is reachable from both directions.
- **`docs/docs/integrations/hooks.md`** — the hooks page lives under `integrations/`, not `core-concepts/`. Add the context seed/read responsibility to the pre-hook and post-hook use-case lists plus a "Per-run framework context" subsection carrying the ordering guarantee: a pre-hook edit is injected this turn, a post-hook edit is persisted (post-hooks run before `store()`). Repeat the tools-use-the-native-handle warning here, since this is the page a hook author lands on.
...
- **Examples** — the three `examples/cli/*_context` demos switch from the raw key to the accessors (`openai_context/demo.py:10,41,42,55`, and the ADK/LangGraph equivalents). No restructure is needed: their session access already sits in a pre-hook and a post-hook while the tools use the native handle (`demo.py:21`, `:30`) — exactly the §1a scope. A fourth demo, `pydanticai_context`, mirrors `openai_context` (tools take `RunContext`, read/write `ctx.deps`); it is the only place the `RunContext`-vs-`ToolContext` distinction is shown working. All four are registered in `.github/test-config.yaml` so CI runs them end to end.
- **`docs/docs/examples/overview.md`** — the CLI Examples inventory lists all four `*_context` demos (grouped, with the native mechanism each uses), and the Supported Frameworks table gains the Pydantic AI and smolagents rows. The pre-existing gaps in the same list (`logfire`, `openai-dynamic`, `openai_structured`, `pydanticai`, `smolagents`, plus a stale `a2a/` entry) are trued up in the same pass. The API Examples list on that page is still incomplete and is left for a follow-up.
