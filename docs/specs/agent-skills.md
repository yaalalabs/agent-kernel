...
| `ak-dev-write-spec` | Writing spec documents for a planned change under `docs/specs/<issue-number>-<short-title>/` in three ordered stages: a concise point-form design spec (`design.md`) reviewed first, then a detailed implementation spec (`spec.md`), then a concise iteration-by-iteration plan (`plan.md`) the PR review checks the code against |
| `ak-dev-review-pr` | Reviewing a PR against architecture, code quality, and testing skills: fetch PR via `gh`, prioritize spec document review (`design.md`/`spec.md`/`plan.md` under `docs/specs/<issue-number>-<short-title>/`, each judged at its own stage's altitude), verify and dedupe findings, post a single batched review with inline comments |
...
| `DEVELOPER_GUIDE.md` | Added "Agent Skills for Contributors" section — lists all 12 dev skills, usage examples |
...
├── ak-dev-sync-skills-and-docs-from-commit/SKILL.md
