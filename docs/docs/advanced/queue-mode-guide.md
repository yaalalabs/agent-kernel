...
2. The client polls `GET /api/v1/chat?request_id=...` to retrieve the result. Same path as the
   `POST`, differentiated by HTTP method — no path segment.
...
| `ECSQueueRequestHandler` | IO container / Thread 1 (REST modes) | FastAPI: `POST /api/v1/chat` enqueues; `GET /api/v1/chat?request_id=...` polls (query param only, no path segment) |
...
- `GET /api/v1/chat?request_id=...` (query param, no path segment) reads from the DynamoDB
  Response Store by `request_id` and returns the result, or `404 NOT_FOUND` if nothing is there
  yet. `request_id` is the only lookup key — a unique UUIDv4 per request, matching the
  serverless Lambda poll path.
