...
Agent Kernel REST API allows the users to add custom routes to the existing REST server. This is a support functioanlity would would avoid users from maintaining a separate REST server for other application work which exposes an endpoint with a configurable prefix `/custom` by default.
