...
- **Air gaps are a first-class case.** One registry override and a per-release image manifest are all a disconnected cluster needs.
...
Two guardrails are baked into the defaults. `minReplicaCount` is 1, because a runner cold start (image pull plus Python imports) is slow enough that scaling from zero would hurt the first customer of the morning. `maxReplicaCount` defaults to the partition count divided by consumers per pod, since past that point a new replica finds no free partition to claim. The `io-handler` tier, which is request-bound, scales on plain CPU.
...
Disconnected clusters get two things. `global.imageRegistry` prefixes every image the chart references, the application images and the Valkey and NATS subcharts alike, so one override redirects the whole release to your private registry. And every release attaches an `images.txt` manifest listing exactly which images to mirror, generated by templating every flavor with every optional tier enabled. The chart itself is an OCI artifact, so it copies into the same registry and installs by digest from there.
