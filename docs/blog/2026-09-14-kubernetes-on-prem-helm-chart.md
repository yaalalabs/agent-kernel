...
Disconnected clusters get two things. `global.imageRegistry` prefixes the application images and the Valkey subchart's; the NATS subchart reads `global.image.registry` instead, so set both and every image in the release comes from your private registry. And every release attaches an `images.txt` manifest listing exactly which images to mirror, generated by templating every flavor with every optional tier enabled. The chart itself is an OCI artifact, so it copies into the same registry and installs by digest from there.
